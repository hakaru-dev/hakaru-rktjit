...
(define (run-test module-env topics words docs output-hs)
  ...
  (define get-index-prob-array-array (jit-get-function (string->symbol (format "get-index$array<array<prob>*>")) module-env))
  (define get-size-prob-array-array (jit-get-function (string->symbol (format "get-size$array<array<prob>*>")) module-env))
  ...
  (define output-list
    (for/list ([i (in-range (get-size-prob-array-array output-c))])
      (define as (get-index-prob-array-array output-c i))
      (for/list ([j (in-range (get-size-prob-array as))])
        (prob2real (get-index-prob-array as i)))))
  (printf "output from prog: ~a\n" output-list)
  (printf "output from hskl: ~a\n" output-hs))

(define env1 (compile-file (build-path current-dir "partial3-2.hkr") empty-nbinfo))

(printf "partial3-2:\n")
(run-test env1 vs-topics vs-words vs-docs '((181.99999999999997 1.0 1.0) (1.0 90.0 1.0) (1.0 1.0 29.99999999999999)))
;; ((27.999999999999996 27.999999999999996 27.999999999999996) (20.000000000000007 20.000000000000007 20.000000000000007) (12.0 12.0 12.0))
