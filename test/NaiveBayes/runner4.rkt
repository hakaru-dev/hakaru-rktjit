...
(define (run-test module-env topics words docs output-hs)
  ...
  (define get-index-array-array-nat (jit-get-function (string->symbol (format "get-index$array<array<nat>*>")) module-env))
  ...
  (define output-list
    (for/list ([i (in-range 4)])
      (define as (get-index-array-array-nat output-c i))
      (for/list ([j (in-range 3)])
        (get-index-nat-array as j))))
  (printf "output from prog: ~a\n" output-list)
  (printf "output from haskell: ~a\n" output-hs))

(define partial-arr4-env (compile-file (build-path current-dir "partial-arr4.hkr") empty-nbinfo))
;; (jit-dump-module partial-arr4-env)

(run-test partial-arr4-env vs-topics vs-words vs-docs '((4 0 1) (2 1 0) (2 1 1) (1 3 1)))
