...
(define (create-test module-env topics words docs output-type)
  ...
  (define zs (rkt->jit module-env '(array nat) (build-list (length words) (const 0))))
  (lambda (word-update)
    (printf "calling prog\n")
    (define init-time (get-time))
    (define output-c (prog topic-prior word-prior num-docs c-words c-docs zs word-update))
