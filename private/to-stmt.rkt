  ...
  (define pass
    (create-rpass
     (expr [(expr-arr t index size body)
            (define fbody (do-arr t index size body))
            (define fvars (set->list (set-remove (find-free-variables fbody) index)))
            (define fname (gensym^ 'af))
            (printf "arr: fname ~a, fvars: ~a\n" fname (map pe fvars))
            (add-to-fns (cons fname (expr-fun fvars t fbody)))
            (expr-app t (expr-intrf fname) fvars)]
           [(expr-sum t index start end body)
            (define fbody (do-sum t index start end body))
            (define fvars (set->list (set-remove (find-free-variables fbody) index)))
            (define fname (gensym^ 'sf))
            (printf "sum: fname ~a, fvars: ~a\n" fname (map pe fvars))
            (add-to-fns (cons fname (expr-fun fvars t fbody)))
            (expr-app t (expr-intrf fname) fvars)]
           [(expr-prd t index start end body)
            (define fname (gensym^ 'pf))
            (define fbody (do-prd t index start end body))
            (define fvars (set->list (set-remove (find-free-variables fbody) index)))
            (printf "prd: fname ~a, fvars: ~a\n" fname (map pe fvars))
            (define fn (expr-fun fvars t fbody))
